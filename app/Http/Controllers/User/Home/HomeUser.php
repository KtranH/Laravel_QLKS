...
class HomeUser extends Controller
{
    ...
    //             : DB::select("SELECT * FROM loaiphong WHERE ISDELETE=1 and(TENLOAIPHONG LIKE ? )", ['%' . $searchTerm . '%', '%' . $searchTerm . '%']);
    //         if (count($data) > 0) {
    //             $output = '<ul>';
    //             foreach ($data as $row) {
    ...
    public function search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $searchTerm = $request->search;
            $data = DB::select(
                "SELECT * FROM loaiphong WHERE ISDELETE=1 
            AND (TENLOAIPHONG LIKE ? OR (SUCCHUA=? AND is_numeric(?)))",
    ...
    public function searchResult($searchTerm)
    {
        ...
        $results = DB::select(
            "SELECT * FROM loaiphong WHERE  TENLOAIPHONG LIKE ? OR SUCCHUA = ?",
            [$likeSearchTerm, $searchTerm]
        );
